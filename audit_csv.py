...
def network(full_path): # full_path from ALL.PY
...
#DEBUG        print("Vuln_Obj_Search_Table DEBUG : ",vuln_Obj_search_table)
...
#DEBUG        print ("Sub_name DEBUG :  ",sub_name)

        # Replace tabulation for member
        #sub_member = table_member[0].replace('\t','')#.split()
        sub_member = table_member[0].splitlines() # Split table into lines
#DEBUG        print ("Sub_member DEBUG : ",sub_member)
        sub_ip = table_ip[0].splitlines()
        #print("::::: debug IP :::::")
        #print(sub_ip)
        #print(vuln_Obj_search_table)
...
#DEBUG                print(sub_name[i])
                for ii in table:
#DEBUG                    print("ii DEBUG : ",ii)
                    for iii in ii:
#DEBUG                        print("iii DEBUG : ", iii)
                        if sub_name[i] in iii:
#DEBUG                            print("Interface already interface_added")
                ...
                if interface_added == False: # If 0 occurence of sub_name in table[]
                    ...
                    #print(table)
#DEBUG                    print("Loop add to table DEBUG :", table)
#DEBUG                    print (sub_name[i], sub_member[i])
#DEBUG                    print (sub_member[i])

#DEBUG    print("Debug last TABLE : ",table)


# SAMPLE #
# MULTI DIMENSIONAL ARRAY PRINT #
    #print("##############################")
    #print("VULNERABLE INTERFACE :")
    #for row in table:
    #    for collumn in row:
    #        for cell in collumn:
    #            print(cell)#,end = " ")
...
def vip(full_path):
    ...
    #print(vuln_Obj_name_table)

    if null_serie not in vuln_Obj_name :
        #print("[!] VIRTUAL IP IS CONFIGURED [!]\n")
        #print(vuln_Obj_name)
        return vuln_Obj_name
        #for i in (range(len(vuln_Obj_name_table))): # Loop for print all objects in the array
#DEBUG                print(sub_name[i])
#            for ii in table:
        #DEBUG                    print("ii DEBUG : ",ii)
#                for iii in ii:
...
#            if vip_added == False: # If 0 occurence of sub_name in table[]
#                table += [[[sub_name[i]],[sub_subnet[i]]]] # Make 2 dimensionnal array for [[Interface name], [Attributes]]
#                #print(table)
...
def ipsec_p1(full_path):
    ...
    for best_proposal in proposal_list:
        ...
        #for configured_proposal in ipsec_Obj_proposal:
        #print("TUNNEL NAME : ", ipsec_Obj_name)
        #print("BEST PROPOSAL : ", best_proposal)
        #print(ipsec_Obj_dhgrp)
        #print(ipsec_Obj_proposal)
        #print("CONFIGURED PROPOSAL : ", configured_proposal)
        #print(ipsec_Obj_proposal)
        #print(range(len(ipsec_Obj_name)))

        if best_proposal not in ipsec_Obj_proposal :
            #print ('[!] VULNERABLE IPSEC %s' % ipsec_Obj_name)
            ...
            for i in (range(len(ipsec_Obj_name))): # Loop for print all objects in the array
                ...
                if ipsec_added == False: # If 0 occurence of sub_name in table[]
                    ...
                    #table += [[[ipsec_Obj_name[i]], [ipsec_Obj_proposal[i]], [ipsec_Obj_dhgrp[i]]]]
                    #print(table)
                    ipsec_vuln = True
        else:
...
def policies(full_path):
    ...
    for value in any_value:
        ...
        if null_serie not in policy_Obj_srcaddr:
            for policy_id in range(len(policy_Obj_srcaddr)) :
                #print("DEBUG POLICY SRC: " + policy_Obj_srcaddr[policy_id])
                #print("DEBUG POLICY UUID : " + policy_Obj_uuid[policy_id])
                #print(policy_id)
    ...
    for value in any_value:
        ...
        if null_serie not in policy_Obj_dstaddr:
            for policy_id in range(len(policy_Obj_dstaddr)) :
                #print("DEBUG POLICY DST : " + policy_Obj_dstaddr[policy_id])
    ...
    #policy_vuln_id = (df['id'].to_string(index=False)).splitlines()
    #policy_Obj_interface = (df['interface'].to_string(index=False)).splitlines()
    #ipsec_Obj_proposal = df[df['proposal'].str.contains(best_proposal) == True].proposal
